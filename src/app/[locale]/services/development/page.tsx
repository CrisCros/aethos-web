...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'meta.serviceDevelopment' })

  return {
    title: { absolute: t('title') },
    description: t('description'),
    alternates: buildAlternates(locale, '/services/development'),
...
export default async function DevelopmentPage({
  ...
  void params
  const t    = await getTranslations('serviceDevelopment')
        ...
        <div className="mx-auto px-5" style={{ maxWidth: '1100px' }}>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              ...
              style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)', color: 'var(--fg)' }}
            >
              <Zap size={14} /> {tNav('automation')} <ArrowRight size={13} />
            </Link>
          </div>
        </div>
      </section>
