...
export default function ServicesGrid() {
  ...
  return (
    <section id="solutions" style={{ padding: 'clamp(64px,10vw,128px) 0', background: 'var(--navy)' }}>
      <div className="mx-auto px-5" style={{ maxWidth: '1240px' }}>
        ...
        >
          <span className="inline-block text-xs font-semibold uppercase tracking-widest mb-3 navy-eyebrow">
            {t('eyebrow')}
          </span>
          <h2 className="font-semibold mb-4 navy-heading" style={{ fontSize: 'clamp(28px,3.6vw,44px)', letterSpacing: '-0.03em', lineHeight: '1.1' }}>
            {t('h2')}
          </h2>
          <p className="navy-body" style={{ fontSize: '17px', lineHeight: '1.55' }}>
            {t('subtitle')}
          </p>
        </motion.div>
        ...
        <div className="bento">
          {CELLS.map(({ id, Icon, title, body, mini: Mini, lg, wide }, i) => (
            <motion.div
              key={id}
              className={`card-interactive flex flex-col gap-3 p-6 rounded-[20px] overflow-hidden bento-cell${lg ? ' bento-lg' : wide ? ' bento-wide' : ''}`}
        ...
        <div className="flex justify-center mt-12">
          <Link
            href="/services"
            className="navy-btn btn inline-flex items-center gap-2 text-sm font-medium px-5 py-2.5 rounded-xl"
